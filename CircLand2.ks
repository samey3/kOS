...
LOCK preLandedCoords TO SHIP:BODY:GEOPOSITIONOF(SHIP:BODY:POSITION - SHIP:POSITION + vecInitialSite).
LOCK landingCoords TO LATLNG(preLandedCoords:LAT, addLongitude(preLandedCoords:LNG, -impactRotationAngle)).
LOCK longitudeDifference TO addLongitude(targetCoordinates:LNG, -landingCoords:LNG).
...
UNTIL (periapsis + BODY:RADIUS) < intersectPeri OR ag9 = True {
	...
	PRINT "Longitude difference : " + longitudeDifference.
	...
	PRINT " ".
	PRINT "Target coords  :".
	PRINT "LAT : " + targetCoordinates:LAT.
	PRINT "LNG : " + targetCoordinates:LNG.
	PRINT " ".
	WAIT 0.1.
}
