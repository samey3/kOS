...
//--------------------------------------------------------------------------/
	...
	RUNPATH("basic_functions/setApoapsis.ks", req_apoapsis).
	...
	RUNPATH("basic_functions/setApoapsis.ks", base_apoapsis).
	
	//Call the node burn here
...
FUNCTION reducePositionDifferences{ //THE FIRST SMOOTH ROTATE
	...
	//High speed stabilization
	LOCAL stopTime IS TIME:SECONDS + 15.
	UNTIL ((rcsX = False AND rcsY = False AND rcsZ = False) OR TIME:SECONDS > stopTime){	
	...
	//Low speed stabilization
	SET stopTime TO TIME:SECONDS + 45. //30 seconds to reduce
	UNTIL ((rcsX = False AND rcsY = False AND rcsZ = False) OR TIME:SECONDS > stopTime){	
