...
//---------------------------------------------/		
	...
	moveToPoint(targetCraft,(SHIP:POSITION - targetCraft:POSITION):NORMALIZED*100,1).
...
moveToPoint(targetPort,targetPort:FACING:VECTOR*100,1).		
...
moveToPoint(targetPort,targetPort:FACING:VECTOR:NORMALIZED * (selfPortDistance),1).
...
FUNCTION moveToPoint {
	...
	//Accelerate to move speed-------------------------------------//
		...
		WAIT UNTIL (moveSpeed - ABS(relative_velocity_CA_Vector:X)) <= 0.1.
		SET SHIP:CONTROL:FORE TO 0.
