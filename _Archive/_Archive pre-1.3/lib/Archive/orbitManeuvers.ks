//Imports setting orbital parameters
runoncepath("lib/setOrbitParameters.ks").
...
FUNCTION circularize {
	...
	//--------------------------------------------------------------------------/
		...
		IF(_radius > _apoapsis){
			setApoapsis(_radius, _precise).
			setPeriapsis(_radius, _precise).
		}
		ELSE {
			setPeriapsis(_radius, _precise).
			setApoapsis(_radius, _precise).
...
FUNCTION basicRendezous {
	...
	//--------------------------------------------------------------------------/	
		...
		ELSE IF ((backwardSep < forwardSep) AND (b_reqApo < SHIP:BODY:SOIRADIUS)){ // AND (SHIP:ORBIT:HASNEXTPATCH = FALSE
			...
			setApoapsis(b_reqApo, FALSE, TRUE).
			KUNIVERSE:TIMEWARP:WARPTO(TIME:SECONDS + SHIP:ORBIT:PERIOD/2).
			WAIT UNTIL WARP = 0 and SHIP:UNPACKED.
			setApoapsis(b_baseApo, FALSE, TRUE).
		}
}
