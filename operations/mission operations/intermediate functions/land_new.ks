//Redo the entire structure
//Optional suicide burn
//Stage changes with altitude (and maybe flameout? Not really likely needed)
//Optional horizontal reduction
//Optional glide corrections
//Add stages to suicideBurn as well

//This intermediate function scripts should be broken up to basic functions:
//deorbit (opt)
//descent stuff
//landing burn (opt)
	...
	@lazyglobal OFF.
	throwEvent(SHIP:BODY:NAME + "_LAND_0").
	CLEARSCREEN.
...
//--------------------------------------------------------------------------/
	...
	//Shouldnt have to define if no horizontal glide/landing burn
	//From the mission script.
	
	//It will go into them anyways and attempt?
	//Just don't put engines on?
...
//--------------------------------------------------------------------------/
	...
	//#######################################################################
		...
		IF(willImpact(SHIP) = FALSE){
			...
			//Transition to target orbit--------------------------|
				RUNPATH("operations/mission operations/circularManeuver.ks", _interceptAltitude, meanShift/(360/SHIP:ORBIT:PERIOD), inclination, FALSE).
		...
		IF(_landingLocation <> 0){		
			...
			//Perform corrections---------------------------------|	
				...
				ELSE{
					...
					RUNPATH("operations/mission operations/modTrajectory.ks", _coordinates, 0, 20).
	...
	//#																		#
	//#######################################################################	
		throwEvent(SHIP:BODY:NAME + "_LAND_4").
