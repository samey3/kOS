...
//LOCAL SMA.
LOCAL extraRotationTime IS ETA:APOAPSIS. //How many extra seconds (To get how much the planet rotates by) the ship must orbit until longitude is close enough
LOCAL bodyRotations TO extraRotationTime/BODY:ROTATIONPERIOD.
...
UNTIL ABS(addLongitude(targetCoordinates:LNG, -addLongitude(landingCoords:LNG, -lngDifference))) < 10 {
	SET extraRotationTime TO extraRotationTime + SHIP:OBT:PERIOD.	
	SET bodyRotations TO (timeToAlt1 + extraRotationTime)/BODY:ROTATIONPERIOD.
	SET lngDifference TO MOD(bodyRotations,1)*360.
}

SET landingCoords TO LATLNG(landingCoords:LAT, addLongitude(landingCoords:LNG, -lngDifference)).
...
FUNCTION addLongitude {
	...
	LOCAL longNew IS angle.
	IF (longitude + angle) > 180 {
		SET longNew TO (-180 + (longitude + angle - 180)).
	}	
	ELSE IF (longitude + angle) < -180 {
		SET angleNew TO (180 + (longitude + angle + 180)).
	}
	ELSE{
		SET longNew TO (longitude + angle).
	}
	RETURN longNew.
}
