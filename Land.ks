	...
	CLEARSCREEN.
	LOCK STEERING TO SHIP:FACING. //Do we need to do this for every single script...? //smoothRotate can always pull its value maybe
	LOCK THROTTLE TO 0.
	UNLOCK STEERING.
	UNLOCK THROTTLE.
...
//--------------------------------------------------------------------------/
	...
	//COULD find a rough requirement of needed DV left to burn


//If in atmo and not fed a vessel/location, exit.


//If orbit, deorbit
//If suborbital, skip deorbit
	...
	//#######################################################################
		...
		//Start burn at time:
		//Plane at landing location, take 'horizontal' distance.
		//(Time for engine at current fuel to fully burn off all surface velocity) x 2
	...
	//#######################################################################
		...
		PRINT("Type  : " + paramObj:TYPENAME).
		PRINT("Value : " + paramObj).
		RUNPATH ("basic_functions/suicideBurn.ks", paramObj).
