...
//--------------------------------------------------------------------------/
	...
	LOCAL shipHeight IS vesselHeight(SHIP)[0] + 15. //Landing legs position is at the top? So doesn't get the full length. Could we perhaps just take the top of the target and use that for distance?
...
//--------------------------------------------------------------------------/
	...
	UNTIL(distanceLeft <= (stopDistance + shipHeight + ABS(VERTICALSPEED*0.02))){
		...
		IF(landingCoordinates <> 0){
			...
			//SET t3 TO VECDRAWARGS(SHIP:POSITION, (50*(-ADDONS:TR:PLANNEDVECTOR:NORMALIZED) + SQRT(impactDif:MAG)*impactDif:NORMALIZED),YELLOW,"Result",1,TRUE).
			
			//SET t4 TO VECDRAWARGS(SHIP:POSITION, ADDONS:TR:PLANNEDVECTOR:NORMALIZED*15,WHITE,"True planned",1,TRUE).
			//SET t5 TO VECDRAWARGS(SHIP:POSITION, impactDif:NORMALIZED*15,WHITE,"True impact diff",1,TRUE).
		}
	}
	...
	UNTIL (v_i < 0.5 OR SHIP:STATUS = "LANDED" OR SHIP:STATUS = "SPLASHED"){
		...
		IF(_landObject <> 0 AND _landObject:ISTYPE("Vessel") AND targetHeightAdded = FALSE AND _landObject:UNPACKED){
			...
			SET shipHeight TO shipHeight + targHeight - targSubmerged.			
