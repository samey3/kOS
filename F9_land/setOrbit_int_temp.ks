...
//--------------------------------------------------------------------------/
	...
	UNTIL(FALSE){
		...
		RUNPATH("F9_land/executeNode_temp.ks", NODE(res["t"], res["radial_1"], res["normal_1"], res["prograde_1"])).
			
		//If the target orbit/object is NOT a body, perform second maneuver
		IF (NOT bodyList:CONTAINS(_orbitObject)){
			RUNPATH("F9_land/executeNode_temp.ks", NODE(res["t"] + res["dt"], res["radial_2"], res["normal_2"], res["prograde_2"])).
		}
	...
	throwEvent(SHIP:BODY:NAME + "_ORBIT_SET").
...
//--------------------------------------------------------------------------/
	...
	FUNCTION isAcceptable {
		...
		//PRINT(scalarDifference(SHIP:ORBIT:INCLINATION, inc) < 0.1).
		//PRINT(scalarDifference(SHIP:ORBIT:ECCENTRICITY, ecc) < 0.01).
		//PRINT((scalarDifference(SHIP:ORBIT:LONGITUDEOFASCENDINGNODE, lan) < 0.1 OR inc < 0.5)).
		//PRINT((scalarDifference(SHIP:ORBIT:ARGUMENTOFPERIAPSIS, argp) < 0.1 OR ecc < 0.05)).
		//PRINT((NOT _matchTrueAnomaly OR scalarDifference(SHIP:ORBIT:TRUEANOMALY, tanm) < 0.05)).	
		...
		IF((percentDifference(SHIP:ORBIT:SEMIMAJORAXIS, sma) < 0.01 //Will be large numbers, thus percentDifference is appropriate
			AND scalarDifference(SHIP:ORBIT:INCLINATION, inc) < 0.1
			AND scalarDifference(SHIP:ORBIT:ECCENTRICITY, ecc) < 0.01
			AND (scalarDifference(SHIP:ORBIT:LONGITUDEOFASCENDINGNODE, lan) < 0.1 OR inc < 0.5) //If under half a degree of inclination, ignore
			AND (scalarDifference(SHIP:ORBIT:ARGUMENTOFPERIAPSIS, argp) < 0.1 OR ecc < 0.05) //If nearly circular, ignore
			AND (NOT _matchTrueAnomaly OR scalarDifference(SHIP:ORBIT:TRUEANOMALY, tanm) < 0.05)
		)){
			RETURN TRUE.
		}
		ELSE {
