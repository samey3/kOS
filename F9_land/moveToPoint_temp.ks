...
//--------------------------------------------------------------------------/
	...
	//Initial approach------------------------------------|
		...
		UNTIL(((endPosition - refPos):MAG <= 1.5*stoppingDistance AND _slowDown = TRUE) OR (endPosition - refPos):MAG <= 0.5){ //Basically as long as we pass it, it exits it... Fix this?
	...
	//----------------------------------------------------\
	//Final approach--------------------------------------|
		//Switches to using only the position PIDs for the final movement (e.g. the slow down?)
