...
//--------------------------------------------------------------------------/
	...
	//Burn parameters	
	SET base_acceleration TO 0.95*SHIP:AVAILABLETHRUST / SHIP:MASS. //Mass in metric tonnes	
		SET burnTime TO _node:DELTAV:MAG / base_acceleration.
		LOCK thrustPercent TO (base_acceleration * SHIP:MASS) / SHIP:AVAILABLETHRUST.
				
	//Burn timing
	SET startTime TO TIME:SECONDS + _node:ETA.
...
//--------------------------------------------------------------------------/
	...
	IF(_node:ETA > 20){
		...
		KUNIVERSE:TIMEWARP:WARPTO(startTime - (burnTime/2 + 20)).
	...
	WHEN (timeLeft <= (burnTime/2 + 20)) THEN { LOCK STEERING TO _node:BURNVECTOR.}
	...
	IF(VANG(_node:BURNVECTOR, SHIP:FACING:VECTOR) > 2){
		PRINT("This one").
	}
	ELSE {
		PRINT("No, this").
	...
	RUNPATH ("basic_functions/modVelocity_node.ks", _node).
